	...
	{	
		...
		else
		{
			if (increase >= m_difference)
			{
				...
				m_difference = 0;
				m_ticket[m_position] = m_ticket[m_position] % TEN;
			}
			else
			{
		...
		if (m_count[countPosition] > NINE)
		{
			m_count[countPosition] = m_count[countPosition] % TEN;
			m_count[countPosition]++;
		}
		m_position--;
		countPosition++;		
	}
...
void CLuckyTicketsCalculation::GetMinNumberOfNextTickets()
{
	m_position = m_ticket.size() - 1;
	...
	for (size_t i = m_position; i > m_ticketHalfLength-1; i--)
	{
		m_count.push_back(0);
	}

...
BOOST_FIXTURE_TEST_SUITE(Check_engine, FilesForTests)
	...
	{
		BOOST_CHECK(!ticket.ReadTicketNumber(fifthtFile));
	}

BOOST_AUTO_TEST_CASE(first_test_from_lecture)
{
	BOOST_CHECK(ticket.ReadTicketNumber(firstFile));
...
{
	BOOST_CHECK(ticket.ReadTicketNumber(secondFile));
...
{
	BOOST_CHECK(ticket.ReadTicketNumber(thirdFile));
...
{
	BOOST_CHECK(ticket.ReadTicketNumber(sixthFile));
...
{
	BOOST_CHECK(ticket.ReadTicketNumber(seventhFile));
...
{
	BOOST_CHECK(ticket.ReadTicketNumber(fourthFile));
